...
$(document).ready( function(){
    sell_table = $('#sell_table').DataTable({
        ...
        serverSide: true,
        aaSorting: [[0, 'desc']],
        ajax: '/events/draft-dt?is_quotation=0',
        columnDefs: [ {
            "targets": 6,
            "orderable": false,
            "searchable": false
    ...
    $('#daterange-btn').daterangepicker(
        ...
        function (start, end) {
            ...
            sell_table.ajax.url( '/events/draft-dt?is_quotation=0&start_date=' + start.format('YYYY-MM-DD') +
                '&end_date=' + end.format('YYYY-MM-DD') ).load();
        }
    );
    $('#daterange-btn').on('cancel.daterangepicker', function(ev, picker) {
        sell_table.ajax.url( '/events/draft-dt?is_quotation=0').load();
