...
<section class="content no-print">
    @component('components.widget', ['class' => 'box-primary'])
        @slot('tool')
            <div class="box-tools">
                <a class="btn btn-block btn-primary" href="{{action('EventPosController@create')}}">
...
$(document).ready( function(){
    sell_table = $('#sell_table').DataTable({
        ...
        serverSide: true,
        aaSorting: [[0, 'desc']],
        ajax: '/events/draft-dt?is_quotation=1',
        columnDefs: [ {
            "targets": 6,
            "orderable": false,
            "searchable": false
    ...
    $('#daterange-btn').daterangepicker(
        ...
        function (start, end) {
            ...
            sell_table.ajax.url( '/events/draft-dt?is_quotation=1&start_date=' + start.format('YYYY-MM-DD') +
                '&end_date=' + end.format('YYYY-MM-DD') ).load();
        }
    );
    $('#daterange-btn').on('cancel.daterangepicker', function(ev, picker) {
        sell_table.ajax.url( '/events/draft-dt?is_quotation=1').load();
